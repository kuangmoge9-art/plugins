...
- If the user asks to create a new Google Doc, route to the Docs skill. The Docs skill must use `[@documents](plugin://documents@openai-primary-runtime)` to create a local `.docx`, keep all local builder scripts and render artifacts in per-task scratch space, then upload and convert it as a native Google Doc and clean up local staging files after connector readback.
